...
Referenced by Model Reviewer and the Delivery Manager when model work is in progress.

---

## When to invoke

- After Model Reviewer has produced a ranked improvements list
...
| Improvement on one dataset only | Flag for Model Reviewer before deciding. Do not merge without review. |
| Improvement on primary metric, regression on another | Flag for Model Reviewer. Trade-offs need explicit human sign-off. |

### 6. Calibrator review
After every 3–5 completed iterations, invoke Model Reviewer to:
...
5. **Model Reviewer reviews periodically, not at every iteration.** Frequent reviews slow the loop without proportionate benefit. Every 3–5 iterations is the right cadence unless a result is ambiguous.
...
- A substitute for Model Reviewer's structural review — the loop refines
